...
**Owned by:** #{event.server.owner.mention}
...
**Currently playing:** #{playing}"
		if !event.channel.private?
			...
			if !event.user.nick.nil?; desc << "\n**Nickname:** #{event.user.nick}"; end
			roles = event.user.roles
			desc << "\n**Roles:** #{if roles.empty?; "No assigned roles." else roles.map {|x| x.name}.join(", "); end}"
		end
